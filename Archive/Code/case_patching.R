...
#test.player.1 <- rep(0.51,19)
#test.player.2 <- rep(0.49,19)
#demo.1 <- rep(0,8)
#for (i in 0:7){
#  print(match.win.prob(test.player.1,test.player.2,format=i))
#  demo.1[i+1] <- match.win.prob(test.player.1,test.player.2,format=i)
#}
#plot(demo.1)

# version of patched function without pointwise probs
...
one.set.win.prob.nonpw <- function(p1,p2,format=0){
  # function accounts for match formats from 2011 onwards!
  # under the iid assumptions, calculates player 1 (P1)'s probability of winning a match
  # with player 2 (P2), where P1 is the player who serves the first game of the first set
  # input:
...
#p2 <- 1-p1
#prob <- rep(0,21)
#for (i in 1:21) prob[i] <- match.win.prob.nonpw(p1[i],p2[i],format=5)
